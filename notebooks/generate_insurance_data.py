...
FIRST_NAMES = [
    ...
    'Salvador', 'Graciela', 'Gerardo', 'Lorena', 'Pablo', 'Teresa', 'Edgar', 'Luz',
    ...
    'Kendrick', 'Janae', 'Demetrius', 'Tiana', 'Xavier', 'Nia', 'Donovan', 'Amara',
    ...
    'Akira', 'Naomi', 'Satoshi', 'Midori', 'Daiki', 'Aiko', 'Kenta', 'Mayu',
    'Min-jun', 'Seo-yeon', 'Ji-hoon', 'Min-ji', 'Sung', 'Hye', 'Joon', 'Eun',
    'Hyun', 'Ji', 'Sang', 'Yoon', 'Jin', 'Soo', 'Young', 'Hee',
    ...
    'Vivek', 'Nisha', 'Pranav', 'Riya', 'Varun', 'Pallavi', 'Akash', 'Swati',
    'Aarav', 'Aadhya', 'Vihaan', 'Ishita', 'Advait', 'Saanvi', 'Reyansh', 'Ananya',
    ...
    'Alexander', 'Avery', 'Sebastian', 'Scarlett', 'Jack', 'Emily', 'Leo', 'Aria'
]
...
LAST_NAMES = [
    ...
    'Porter', 'Spencer', 'Weaver', 'Lawrence', 'Elliott', 'Fuller', 'Fowler', 'Hanson',
    'Pierce', 'Harvey', 'Arnold', 'Perkins', 'Ferguson', 'Palmer', 'Matthews', 'Wagner',
    ...
    'Escobar', 'Mejia', 'Rosales', 'Fuentes', 'Ochoa', 'Alvarado', 'Navarro', 'Cortez',
    'Serrano', 'Valencia', 'Figueroa', 'Solis', 'Cisneros', 'Gallegos', 'Montoya', 'Duran',
    'Washington', 'Jefferson', 'Franklin', 'Banks', 'Booker', 'Brooks', 'Dixon', 'Gaines',
    'Grant', 'Hawkins', 'Hunter', 'Jefferson', 'Jordan', 'Lawrence', 'Lawson', 'Mack',
    'Mosley', 'Payne', 'Pryor', 'Simmons', 'Singleton', 'Tate', 'Terrell', 'Wade',
    ...
    'Saito', 'Kato', 'Yamada', 'Yoshida', 'Sasaki', 'Yamaguchi', 'Matsumoto', 'Inoue',
    ...
    'Trivedi', 'Chatterjee', 'Banerjee', 'Mukherjee', 'Das', 'Sen', 'Ghosh', 'Bose',
    'Hassan', 'Ali', 'Mohammed', 'Ibrahim', 'Abdullah', 'Rahman', 'Khalil', 'Saleh',
    'Ahmad', 'Youssef', 'Haddad', 'Khoury', 'Mansour', 'Nasser', 'Farouk', 'Hamid',
    'Salem', 'Rashid', 'Saeed', 'Mustafa', 'Omar', 'Karim', 'Amin', 'Habib',
    'Mueller', 'Schmidt', 'Schneider', 'Fischer', 'Weber', 'Meyer', 'Wagner', 'Becker',
    'Schulz', 'Hoffmann', 'Koch', 'Richter', 'Klein', 'Wolf', 'Schroeder', 'Neumann',
    'Schwartz', 'Zimmerman', 'Braun', 'Krueger', 'Hofmann', 'Hartmann', 'Lange', 'Werner',
    'Rossi', 'Russo', 'Ferrari', 'Esposito', 'Romano', 'Colombo', 'Ricci', 'Marino',
    'Greco', 'Bruno', 'Gallo', 'Conti', 'DeLuca', 'Costa', 'Giordano', 'Mancini',
    'Rizzo', 'Lombardi', 'Moretti', 'Barbieri', 'Fontana', 'Santoro', 'Mariani', 'Rinaldi',
    'Byrne', 'Doyle', 'Lynch', 'Quinn', 'Brennan', 'Gallagher', 'Flynn', 'Casey'
]


class SimpleFaker:
    """Simple name generator to replace Faker library."""
...
CONFIG = {
    ...
    # Note: negative correlation between quality and cost (email cheap but low quality)
    'channels': {
        'paid_search': {
    ...
    # Premium and claims settings (for sold policies)
    'base_annual_premium': {
        'Health': {'mean': 4800, 'std': 1200},
        'Life': {'mean': 1800, 'std': 600},
        'Property_Casualty': {'mean': 2400, 'std': 700},
    },
    ...
    'avg_claim_severity': {
        'Health': {'mean': 8500, 'std': 12000},
        'Life': {'mean': 250000, 'std': 100000},  # Death benefit
        'Property_Casualty': {'mean': 15000, 'std': 25000},
    },

    # Base expected tenure in years (will be modified by demographics)
    'base_tenure': {
        'Health': {'mean': 3.2, 'std': 1.5},
        'Life': {'mean': 8.5, 'std': 4.0},
        'Property_Casualty': {'mean': 4.5, 'std': 2.0},
    },
...
def simulate_policy_economics(
    ...
    """
    Simulate premium, tenure, and claims for a sold policy.

    Key insight: Lower quality channels (cheaper CPL) attract higher-risk customers
    who have more claims and shorter tenure. This demonstrates adverse selection.
    """
    channel_quality = CONFIG['channels'][channel]['quality_score']
    credit_idx = CONFIG['credit_scores'].index(demographics['credit_score'])
    income_idx = CONFIG['income_brackets'].index(demographics['income_bracket'])
    age = demographics['age']

    # --- ANNUAL PREMIUM ---
    base_premium = CONFIG['base_annual_premium'][product]
    premium = np.random.normal(base_premium['mean'], base_premium['std'])

    # Age affects premium (especially life and health)
    if product == 'Life':
        # Life insurance premium increases significantly with age
        age_premium_modifier = 0.5 + (age / 50)  # Younger = cheaper
    elif product == 'Health':
        age_premium_modifier = 0.6 + (age / 60)
    else:
        age_premium_modifier = 0.9 + (age / 200)  # Slight age effect for P&C

    # Credit affects premium (lower credit = higher premium in most states)
    credit_premium_modifier = 1.15 - (credit_idx * 0.05)  # Poor credit pays more

    annual_premium = premium * age_premium_modifier * credit_premium_modifier
    annual_premium = round(max(annual_premium, 500), 2)

    # --- EXPECTED TENURE ---
    base_tenure = CONFIG['base_tenure'][product]
    tenure = np.random.normal(base_tenure['mean'], base_tenure['std'])

    # Higher quality channels = longer tenure (better fit customers)
    tenure_channel_modifier = 0.7 + (channel_quality * 0.6)

    # Better credit = longer tenure (more stable customers)
    tenure_credit_modifier = 0.8 + (credit_idx * 0.15)

    # Higher income = longer tenure
    tenure_income_modifier = 0.85 + (income_idx * 0.05)

    expected_tenure = tenure * tenure_channel_modifier * tenure_credit_modifier * tenure_income_modifier
    expected_tenure = round(max(expected_tenure, 0.5), 2)  # Minimum 6 months

    # --- CLAIMS SIMULATION ---
    base_claim_rate = CONFIG['base_claim_rate'][product]

    # KEY INSIGHT: Channel quality inversely affects claim rate
    # Lower quality channels attract adverse selection - people who know they need insurance
    claim_rate_channel_modifier = 1.4 - (channel_quality * 0.6)  # Low quality = higher claims

    # Credit score strongly predicts claims (well-documented in insurance)
    claim_rate_credit_modifier = 1.5 - (credit_idx * 0.2)  # Poor credit = more claims

    # Age affects claims differently by product
    if product == 'Health':
        claim_rate_age_modifier = 0.6 + (age / 80)  # Older = more health claims
    elif product == 'Life':
        claim_rate_age_modifier = 0.3 + (age / 60)  # Older = higher mortality
    else:  # P&C
        # Young drivers have more accidents, then decreases, slight uptick for elderly
        if age < 25:
            claim_rate_age_modifier = 1.4
        elif age < 65:
            claim_rate_age_modifier = 0.9
        else:
            claim_rate_age_modifier = 1.1

    adjusted_claim_rate = base_claim_rate * claim_rate_channel_modifier * claim_rate_credit_modifier * claim_rate_age_modifier
    adjusted_claim_rate = min(adjusted_claim_rate, 0.95)  # Cap at 95%

    # Simulate if claim occurred (using tenure period)
    # For simplicity, we'll track: any claim, early claim (first year), claim count, total claim amount
    has_claim = False
    has_early_claim = False  # Claim within first 12 months
    claim_count = 0
    total_claim_amount = 0.0

    # Simulate claims over tenure period
    years_in_tenure = int(np.ceil(expected_tenure))
    severity = CONFIG['avg_claim_severity'][product]

    for year in range(years_in_tenure):
        if np.random.random() < adjusted_claim_rate:
            has_claim = True
            claim_count += 1
            claim_amount = max(0, np.random.normal(severity['mean'], severity['std']))
            total_claim_amount += claim_amount

            if year == 0:
                has_early_claim = True

    total_claim_amount = round(total_claim_amount, 2)

    # --- CALCULATED METRICS ---
    total_premium = annual_premium * expected_tenure
    expected_value = total_premium - total_claim_amount  # Simplified expected value
    loss_ratio = total_claim_amount / total_premium if total_premium > 0 else 0

    return {
        'annual_premium': annual_premium,
        'expected_tenure_years': expected_tenure,
...
def simulate_funnel_progression(
    ...
    For sold policies, also includes premium, tenure, and claims data.
    """
    ...
    result = {
        ...
        'final_status': 'lead',
        'ltv': None,
        # Policy economics fields (only populated for sold policies)
        'annual_premium': None,
        'expected_tenure_years': None,
        'total_premium': None,
        'has_claim': None,
        'has_early_claim': None,
        'claim_count': None,
    ...
    for i, (stage, stage_key) in enumerate(zip(stages, stage_keys)):
        ...
        if np.random.random() < prob:
            ...
            if stage == 'sold':
                ...
                # Simulate policy economics (premium, tenure, claims)
                policy_econ = simulate_policy_economics(
                    product, demographics, channel, current_date
                )
                result.update(policy_econ)
        else:
...
def generate_lead_detail_data(all_leads: List[Dict]) -> pd.DataFrame:
    ...
    For sold policies, includes premium, tenure, and claims data.
    """
    lead_details = []
    ...
    for lead in all_leads:
        ...
        for product in products:
            ...
            lead_details.append({
                ...
                # Policy economics (only populated for sold policies)
                'annual_premium': funnel_result['annual_premium'],
                'expected_tenure_years': funnel_result['expected_tenure_years'],
                'total_premium': funnel_result['total_premium'],
                'has_claim': funnel_result['has_claim'],
                'has_early_claim': funnel_result['has_early_claim'],
                'claim_count': funnel_result['claim_count'],
                'total_claim_amount': funnel_result['total_claim_amount'],
                'expected_value': funnel_result['expected_value'],
                'loss_ratio': funnel_result['loss_ratio'],
            })
...
def generate_all_data(output_dir: str = '.'):
    ...
    # Claims summary for sold policies
    sold_policies = leads_detail_df[leads_detail_df['sold_date'].notna()]
    if len(sold_policies) > 0:
        claims_count = sold_policies['has_claim'].sum()
        early_claims_count = sold_policies['has_early_claim'].sum()
        total_premiums = sold_policies['total_premium'].sum()
        total_claims = sold_policies['total_claim_amount'].sum()
        avg_loss_ratio = total_claims / total_premiums if total_premiums > 0 else 0

        print(f"\n  Claims Summary (sold policies):")
        print(f"  - Policies with claims: {claims_count:,} ({claims_count / len(sold_policies) * 100:.1f}%)")
        print(
            f"  - Early claims (year 1): {early_claims_count:,} ({early_claims_count / len(sold_policies) * 100:.1f}%)")
        print(f"  - Total Premium: ${total_premiums:,.2f}")
        print(f"  - Total Claims: ${total_claims:,.2f}")
        print(f"  - Overall Loss Ratio: {avg_loss_ratio:.1%}")
